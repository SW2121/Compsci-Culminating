...
def gotoQuiz(screen):
    from quizquestions import run_quiz
    run_quiz()

def gotoTestPassword(screen):
    with open('passwordinput.py') as file2:
        exec(file2.read(), globals())

def gotoRetrievePassphrase(screen):
    from Passphrase_saver import run_passphrase_retriever
    run_passphrase_retriever()
